...
1. **Gap exclusion**: Cases with gaps exceeding 5 years between consecutive visits were excluded (55 cases removed; controls retained regardless, as temporal continuity is only relevant for cases whose visits are aligned to a diagnosis date). Remaining: 10,348 patients (488 cases, 9,860 controls), 1,300,421 visits.
2. **Visit subsampling**: To limit computational burden and prevent overrepresentation of frequently-seen patients, visits were subsampled to a maximum of 20 per patient, preserving the first and last encounters (remaining: 160,192 visits).
...
After all filtering, 181 cases remained for the any-narcolepsy model (from 488 after gap exclusion), 68 for the NT1-only model, and 113 for the NT2/IH-only model, with 9,860 controls. The substantial case attrition reflects the requirement for sufficient clinical documentation in the narrow 2-year pre-diagnostic training window (eTable 1).
...
To characterize the temporal evolution of the risk score relative to diagnosis, we scored all visits within a 5-year pre-diagnostic window using the final model. For diagnosed patients, time was aligned to the date of diagnosis, and we computed the patient-level mean risk score within 1.5-year sliding windows (step size 0.1 years), averaging first within each patient and then across patients. We estimated 95% confidence intervals via bootstrap resampling (200 iterations). For controls, temporal alignment is not meaningful because they have no diagnosis date; we therefore summarized control scores as a single patient-level mean with bootstrap 95% CI across all visits. Time-varying AUC was computed within 1-year sliding windows, comparing case patient-level mean scores in each window against all control patient-level mean scores.
...
The longitudinal prediction cohort initially comprised 10,403 patients with 1,308,867 clinical visits: 543 narcolepsy cases (266 NT1, 277 NT2/IH) and 9,860 controls. After sequential filtering (gap exclusion removing 55 cases with >5-year visit gaps, visit subsampling to max 20 per patient, and temporal windowing with h = 0.5 year horizon exclusion), 181 cases remained for the any-narcolepsy model, 68 for the NT1-only model, and 113 for the NT2/IH-only model, with 9,860 controls (eTable 1). We restricted LOSO cross-validation to sites with at least 50 controls in the held-out fold (BIDMC and MGB); BCH, Emory, and Stanford were excluded from LOSO evaluation due to insufficient control sample sizes (eTable 2).
...
The longitudinal predictive model demonstrated robust discrimination for all three outcomes using pre-diagnostic clinical data (eFigure 9). For the any-narcolepsy model (181 cases, 9,860 controls), stratified 5-fold cross-validation yielded a mean AUC of 0.846 (range across folds: 0.816-0.898) and mean AUPRC of 0.395 (range: 0.303-0.490). LOSO cross-validation (restricted to BIDMC and MGB, which had sufficient controls) produced a mean AUC of 0.772 (range: 0.676-0.868) and mean AUPRC of 0.234 (range: 0.159-0.309), confirming generalizability across institutions (eTable 2).

For the NT1-only model (68 cases, 9,860 controls), 5-fold cross-validation yielded a mean AUC of 0.839 (range: 0.774-0.899) and mean AUPRC of 0.401 (range: 0.174-0.604). LOSO cross-validation yielded a mean AUC of 0.730 (range: 0.602-0.858) and mean AUPRC of 0.229 (range: 0.207-0.251).

For the NT2/IH-only model (113 cases, 9,860 controls), 5-fold cross-validation yielded a mean AUC of 0.820 (range: 0.770-0.867) and mean AUPRC of 0.314 (range: 0.196-0.474). LOSO cross-validation yielded a mean AUC of 0.722 (range: 0.612-0.833) and mean AUPRC of 0.234 (range: 0.156-0.313).

In LOSO evaluation, BIDMC showed stronger performance (AUC 0.868 for any-narcolepsy, 0.858 for NT1, 0.833 for NT2/IH) compared with MGB (AUC 0.676, 0.602, and 0.612, respectively), reflecting differences in cohort composition and clinical documentation practices.
...
Feature evolution heatmaps revealed distinct temporal accumulation patterns differentiating cases from controls (eFigure 12). Features with positive coefficients — stimulant medications, narcolepsy-related keywords, and sleep study references — showed progressive accumulation in cases approaching diagnosis while remaining near-absent in controls (63 features retained after L1 regularization; 232 cases, 232 matched controls with 5 or more visits). Features with negative coefficients, representing general medical terms more prevalent in the broader clinical population, accumulated more rapidly in controls. These patterns were consistent in the NT1-only model (80 features; 88 cases, 88 matched controls), with cataplexy-related features and NT1-specific medications showing particularly strong case-control divergence (eFigure 13).

***Longitudinal Prediction: Clinical Utility***

Using this longitudinal model, the risk score enriches screened populations for narcolepsy cases 62- to 125-fold over the population base rate (Figure 3). At an assumed prevalence of 0.08%, the baseline NNT without any screening is 1,250 (i.e., 1 in 1,250 individuals in the general population has narcolepsy). For the any-narcolepsy model, applying a score threshold of 0.97 yielded an NNT of 20 with a sensitivity of 71%, representing a 62.5-fold enrichment over the population base rate. At a more stringent threshold of 0.99, the NNT decreased to approximately 10 (sensitivity 70%; ~125-fold enrichment). For the NT1-only model, a threshold of 0.97 achieved an NNT of 20 with a sensitivity of 83%, and a threshold of 0.99 achieved an NNT of approximately 10 with a sensitivity of 83%. These results indicate that the model can identify a high-risk subpopulation in which confirmatory diagnostic testing would have a substantially higher yield than unselected screening.

**Discussion**

***Summary of Principal Findings***

To our knowledge, these results are the first multi-site demonstration that pre-diagnostic narcolepsy screening can be automated from routine clinical notes. Using EHR data from 5 academic medical centers, our cross-sectional classifiers achieved AUROC values of 0.997 for NT1, 0.988 for NT2/IH, and 0.990 for any narcolepsy, demonstrating that narcolepsy-related content in clinical notes can be reliably identified across all three classification tasks. Our longitudinal predictive models detected elevated narcolepsy risk months to years before formal diagnosis, with AUC of 0.846 (any narcolepsy), 0.839 (NT1), and 0.820 (NT2/IH) in cross-validation, and a pre-diagnostic signal detectable from at least 5 years before diagnosis that strengthens progressively. NNT analysis showed that the model achieves 62.5- to 125-fold enrichment over the population base rate at clinically practical thresholds, providing the operating parameters needed for EHR-integrated narcolepsy screening.
...
For the longitudinal model, LOSO evaluation was restricted to sites with at least 50 controls (BIDMC and MGB). AUC ranged from 0.676 (MGB) to 0.868 (BIDMC) for any-narcolepsy, and from 0.602 (MGB) to 0.858 (BIDMC) for NT1. This variability reflects differences in cohort composition, patient populations, and documentation practices; deployment in new settings should be accompanied by local calibration or prospective evaluation.
...
Figure 2. Pre-Diagnostic Risk Scores Show Sustained Elevation and Progressive Divergence From Controls Across the 5-Year Pre-Diagnostic Window. (A–C) Risk score trajectories for cases (blue) and controls (orange). Case scores are aligned to the time of diagnosis; bold lines represent the patient-level mean with shaded 95% bootstrap confidence intervals, computed within 1.5-year sliding windows. Control scores are shown as a flat line representing the overall patient-level mean (with 95% bootstrap CI), since time alignment is not meaningful for non-diagnosed patients. (A) Any-narcolepsy model (483 cases, 9,860 controls). (B) NT1 model (239 cases, 9,860 controls). (C) NT2/IH model (244 cases, 9,860 controls). For the any-narcolepsy and NT1 models, mean case risk scores rise progressively from 5 years before diagnosis to above 0.95 near diagnosis. For the NT2/IH model, case scores rise from approximately 0.55 to above 0.90. Control scores remain stable near 0.25 across all models. (D–F) Time-varying AUROC within 1-year sliding windows, comparing case scores in each window against all control scores. Horizontal dashed lines mark AUROC = 0.8 and 0.9. (See figures/figure2_risk_score_trajectories.png)

Figure 3. Risk Score Thresholds Achieve 62.5- to 125-Fold Enrichment Over Population Base Rate. NNT (blue, left y-axis, log scale) and sensitivity (red dashed, right y-axis) as a function of risk score threshold for the any-narcolepsy model (A; 181 cases, 9,860 controls), NT1-only model (B; 68 cases, 9,860 controls), and NT2/IH-only model (C; 113 cases, 9,860 controls). NNT was computed under an assumed population prevalence of 0.08% (1 in 1,250) using sensitivity and specificity from the final model. Annotated operating points are shown at NNT = 10 and NNT = 20 for each model. (See figures/figure3_nnt_analysis.png)
...
eFigure 2. CONSORT Diagram -- Longitudinal Prediction Pipeline. The initial cohort of 10,403 patients (543 narcolepsy cases, 9,860 controls) was filtered through gap exclusion (removing 55 cases with >5-year visit gaps; controls retained regardless), visit subsampling (max 20 per patient), sparse feature removal, and temporal windowing (training window -2.5 to -0.5 years before diagnosis). Final cohorts comprised 181 any-narcolepsy cases, 68 NT1 cases, and 113 NT2/IH cases, with 9,860 controls. All three outcome models were evaluated using 5-fold and LOSO cross-validation. (See figures/efigure2_consort_longitudinal.png)
...
eFigure 9. Longitudinal Models Achieve AUC 0.82-0.85 Across Three Narcolepsy Outcomes. AUC (left column) and AUPRC (right column) for the any-narcolepsy model, NT1-only model, and NT2/IH-only model. Results are shown for stratified 5-fold cross-validation (blue), leave-one-site-out cross-validation (orange), and resubstitution on the final model (green). Individual dots represent per-fold (5-fold CV) or per-site (LOSO) performance. All models used a 0.5-year horizon exclusion, restricting training data to visits occurring 6 months to 2.5 years before diagnosis. The any-narcolepsy model included 181 cases, the NT1-only model 68 cases, and the NT2/IH-only model 113 cases, each with 9,860 controls. (See figures/efigure9_predictive_performance.png)
...
eFigure 12. Narcolepsy-Associated Features Accumulate Progressively in Cases But Not Controls. Heatmaps showing mean cumulative feature values over the 2.5-year pre-diagnostic window for the 63 features with non-zero L1 coefficients in the any-narcolepsy model. Left panel: cases (n = 232); right panel: matched controls (n = 232). Only patients with 5 or more visits were included. Feature values were z-score normalized. Rows are ordered by model coefficient value; red rows indicate positive coefficients and blue rows indicate negative coefficients. (See figures/efigure12_feature_heatmap_any_narcolepsy.png)

eFigure 13. Cataplexy-Related Features and NT1-Specific Medications Show Strong Case-Control Divergence. Same as eFigure 12, for the 80 features with non-zero L1 coefficients in the NT1-only model (88 cases, 88 matched controls with 5 or more visits). Cataplexy-related features, NT1-specific medications, and hypocretin/orexin mentions show particularly strong case-control divergence. (See figures/efigure13_feature_heatmap_nt1.png)
...
| After gap exclusion (>5 yr, cases only) | 10,348 | 1,300,421 | 488 | 241 | 247 | 9,860 |
| After visit subsampling (max 20) | 10,348 | 160,192 | 488 | 241 | 247 | 9,860 |
| After temporal window (h = 0.5 yr) | 10,041 | — | **181** | **68** | **113** | **9,860** |

*Note: Gap exclusion removed 55 cases (25 NT1, 30 NT2/IH) with >5-year gaps between consecutive visits; controls were retained regardless, as temporal continuity is only relevant for cases whose visits are aligned to a diagnosis date. Case attrition from 488 to 181 (any narcolepsy), from 241 to 68 (NT1), and from 247 to 113 (NT2/IH) at the temporal windowing step reflects the requirement that cases have clinical visits within the pre-diagnostic training window (2.5 to 0.5 years before diagnosis). Cases without sufficient documentation in this window were excluded. Controls were not affected by temporal windowing.*
...
| ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- |
| BIDMC | 44 | 11 | 33 | 4,976 | 0.868 | 0.309 | 0.858 | 0.251 | 0.833 | 0.313 |
| MGB | 54 | 26 | 28 | 4,882 | 0.676 | 0.159 | 0.602 | 0.207 | 0.612 | 0.156 |
| **Mean** | — | — | — | — | **0.772** | **0.234** | **0.730** | **0.229** | **0.722** | **0.234** |

*Note: LOSO evaluation was restricted to sites with at least 50 controls in the held-out fold. BCH, Emory, and Stanford were excluded due to insufficient control sample sizes (<50 controls each). Total cohort across all sites comprised 181 any-narcolepsy cases, 68 NT1 cases, and 113 NT2/IH cases with 9,860 controls.*
