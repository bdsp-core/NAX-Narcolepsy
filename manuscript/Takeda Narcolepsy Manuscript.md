...
Random Forest achieved the highest AUROC (0.996) and AUPRC (0.935) for NT1 classification; its ensemble averaging apparently provides robust discrimination despite class imbalance. For NT2/IH, XGBoost achieved the best balance of AUROC (0.977) and AUPRC (0.676), with higher sensitivity than RF. For the combined any-narcolepsy task, XGBoost also performed best (AUROC 0.992, AUPRC 0.934), with performance approaching the NT1-only model. All four classifiers achieved comparable overall performance across tasks, with differences in sensitivity-specificity tradeoffs reflecting their distinct inductive biases.
...
eFigure 9. Predictive Model Performance. AUC (left column) and AUPRC (right column) for the any-narcolepsy model, NT1-only model, and NT2/IH-only model. Results are shown for stratified 5-fold cross-validation (blue), leave-one-site-out cross-validation (orange), and resubstitution on the final model (green). Individual dots represent per-fold (5-fold CV) or per-site (LOSO) performance. All models used a 0.5-year horizon exclusion, restricting training data to visits occurring 6 months to 2.5 years before diagnosis. The any-narcolepsy model included 196 cases, the NT1-only model 66 cases, and the NT2/IH-only model 130 cases, each with 11,049 controls. (See figures/efigure9_predictive_performance.png)

eFigure 10. Risk Score Distributions. Density histograms of patient-level mean risk scores from the final model, shown separately for cases (blue) and controls (orange) for each of the three outcome models. Controls cluster near zero, while cases are broadly distributed with a prominent peak near 1.0. (See figures/efigure10_risk_score_distributions.png)
